<div class="top-bar-area bg-dark text-light">
    <div class="container">
        <div class="row">
            <div class="col-md-4 logo-box">
                <div class="logo">
                    <a href="index.html"><img src="assets/img/logo-light.png" alt="Thumb"></a>
                </div>
            </div>
            <div class="col-md-8 address-info text-right">
                <div class="info">
                    <ul>
                        <li>
                            <i class="fas fa-copy"></i> Total courses: <strong>23400</strong>
                        </li>
                        <li>
                            <i class="fas fa-user-shield"></i> Instructor: <strong>655</strong>
                        </li>
                        <li>
                            <i class="fas fa-phone"></i> Help Line: <strong>+123 456 7890</strong>
                        </li>
                    </ul>
                </div>
            </div>

        </div>
    </div>
</div>
<header id="home">


    <!-- Start Navigation -->
    <nav class="navbar navbar-default logo-less small-pad navbar-sticky bootsnav">

        <div class="container">

            <!-- Start Atribute Navigation -->
            <div class="attr-nav inc-btn">
                <ul>
                    <li>
                        <a href="#">Username</a>
                    </li>
                    <li>
                        <a href="#">PROFILE</a>
                    </li>
                </ul>
            </div>
            <!-- End Atribute Navigation -->



            <!-- Collect the nav links, forms, and other content for toggling -->
            <div class="collapse navbar-collapse" id="navbar-menu">
                <ul class="nav navbar-nav navbar-left" data-in="#" data-out="#">
                    <li>
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">HOME</a>
                    </li>
                    <li>
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">REGION</a>
                    </li>
                    <li>
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">ETABLISSEMENT</a>
                    </li>
                    
                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">AJOUTER EMPLOYEE</a>
                        <ul class="dropdown-menu">
                            <li><a href="">GERANT CENTRAL</a></li>
                            <li><a href="">GERANT REGIONAL</a></li>
                            <li><a href="">GERANT LOCAL</a></li>
                            <li><a href="">INTERVENANT</a></li>
                        </ul>
                    </li>
                    <li>
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">COMPETENCE</a>
                    </li>
                    <li>
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">CATEGORY</a>

                    </li>
                    <li>
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown"> FORMATION</a>

                    </li>
                    <li>
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown"> CERTIFICAT</a>

                    </li>
                </ul>
            </div><!-- /.navbar-collapse -->
        </div>

    </nav>
    <!-- End Navigation -->

</header>
